...
export const StudentManagementDrawer: React.FC<StudentManagementDrawerProps> = ({
  ...
  const handleToggleAllEnrolled = () => {
    if (selectedEnrolledIds.length === enrolledStudents.length) {
      setSelectedEnrolledIds([]);
    } else {
  ...
  const handleRemoveMultipleStudents = async () => {
    if (!course || selectedEnrolledIds.length === 0) return;
    ...
    try {
      ...
      if (successCount > 0) {
        setSuccessMessage(`${successCount} student(s) removed successfully!`);
      }
      if (failCount > 0) {
        setError(`${failCount} student(s) could not be removed.`);
      }
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between mb-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={selectedEnrolledIds.length === enrolledStudents.length}
                    onChange={handleToggleAllEnrolled}
                    className="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-2 focus:ring-blue-500"
                  />
                  <span className="text-sm font-medium text-slate-700">Select All</span>
                </label>
                <p className="text-xs text-slate-500">{enrolledStudents.length} student{enrolledStudents.length !== 1 ? 's' : ''} enrolled</p>
              </div>
              ...
              {selectedEnrolledIds.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3 flex items-center justify-between">
                  <span className="text-sm font-medium text-red-900">
                    {selectedEnrolledIds.length} student{selectedEnrolledIds.length !== 1 ? 's' : ''} selected
                  </span>
                  <button
                    onClick={handleRemoveMultipleStudents}
                    disabled={isRemoving}
              ...
              {sortedEnrolledStudents.map((student) => {
                ...
                const isSelected = selectedEnrolledIds.includes(student.id);
                return (
                  <div 
                    key={student.id} 
                    className={`bg-slate-50 rounded-lg p-4 flex items-center gap-3 ${
                      isSelected ? 'border-2 border-red-500 bg-red-50' : 'border border-slate-200'
                    }`}
                  >
                    <input
                      type="checkbox"
                      checked={isSelected}
                      onChange={() => handleToggleEnrolled(student.id)}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        disabled={loadingStudentId === student.id || isRemoving}
